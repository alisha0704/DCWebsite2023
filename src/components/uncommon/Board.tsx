...
const Board: React.FC<BoardProps> = () => {
  ...
  return (
    <div className="flex justify-center items-center h-3/4  bg-black">
      ...
      <div className="mb-4">
          <h1 className="text-3xl mb-1 mt-8 text-white">The Board</h1>
        </div>
        <Link href="/" className="absolute top-16 left-14 text-white mt-20 ml-8 pl-16 pt-8">
          <FiArrowLeft className="text-5xl" /> { FiArrowLeft}
        </Link>
        <p className="text-md mb-8 text-gray-300">
          Alone you can do so little, together you can do so much. Presenting the Board of Dance Club 2023-24.
        </p>
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4 py-8 w-full">
          {images.map((image, index) => (
            <div key={index} className="text-center">
              ...
              <h2 className="text-lg font-semibold text-white mt-2">{image.memberName}</h2>
              <h3 className="text-sm text-gray-400 mt-1 mb-8">{image.memberDesgn}</h3>
              </div>
            </div>
          ))}
        </div>
